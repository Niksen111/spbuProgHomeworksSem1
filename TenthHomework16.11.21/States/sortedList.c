...
SortedList* createNewElement(Value value, int nodeNumber, SortedList* next)
{
    ...
    newElement->nodeNumber = nodeNumber;
    return newElement;
}

int push(SortedList** head, Value value, int nodeNumber)
{
    if (sortedListIsEmpty(*head) || (*head)->value >= value)
    {
        *head = createNewElement(value, nodeNumber, *head);
    ...
    while (currentElement->next != NULL)
    {
        ...
        {
            currentElement->next = createNewElement(value, nodeNumber, currentElement->next);
            return currentElement->next == NULL ? -1 : 0;
        }
        currentElement = currentElement->next;
    }
    currentElement->next = createNewElement(value, nodeNumber, NULL);
    return currentElement->next == NULL ? -1 : 0;
}
...
int getNodeNumberFromHead(SortedList* head)
{
    return head->nodeNumber;
}